...
export default function MovieModal({
  ...
  // Track active watching duration (must watch for at least 60 seconds / 1 minute to appear in Continue Watching)
  ...
  // Reset or initialize player and restore saved progress when movie shifts
  useEffect(() => {
    watchSecondsRef.current = 0;
    const existing = getContinueWatchingList().some((item) => item.movieId === movie.id);
    setHasReachedThreshold(existing);

    const continueItem = getContinueWatchingList().find((item) => item.movieId === movie.id);

    if (initialPlayerType) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      // Check if 60 seconds (1 minute) threshold reached
      if (watchSecondsRef.current >= 60 && !hasReachedThreshold) {
        setHasReachedThreshold(true);
      }
    }, 1000);

    return () => clearInterval(timer);
  }, [playerType, hasReachedThreshold]);

  // Record watching progress to "Continuar Assistindo" only after watching for at least 1 minute (60s) or if already in list
  useEffect(() => {
    if (playerType === "none" || playerType === "trailer") {
      return;
    }
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto overflow-x-hidden bg-black/95 backdrop-blur-md flex justify-center items-start sm:items-center p-0 sm:p-4 md:p-6 animate-fade-in" id={`movie-modal-overlay-${movie.id}`}>
      ...
      <div className="relative w-full min-h-screen sm:min-h-0 sm:max-h-[92vh] sm:my-auto max-w-5xl bg-[#080808] sm:border sm:border-gray-900/80 sm:rounded-2xl overflow-y-auto shadow-2xl flex flex-col scrollbar-thin" id="modal-scroll-container">
        ...
        <div className="w-full flex flex-col">
              ...
              <div className="w-full h-full relative flex items-end justify-start min-h-[360px] sm:min-h-[400px] md:min-h-[460px]">
                ...
                <div className="relative z-20 p-4 sm:p-8 md:p-10 w-full flex flex-col md:flex-row gap-4 sm:gap-6 md:gap-10 items-start md:items-end">
                  ...
                  <div className="flex-grow min-w-0 w-full">
                    ...
                    {movie.originalTitle && (
                      <span className="text-xs sm:text-sm text-gray-400 italic block mb-2 font-sans">
          ...
          {playerType !== "none" && (
            <div className="bg-[#0b0b0b] border-b border-gray-950 px-4 sm:px-5 py-3 flex flex-col sm:flex-row items-center justify-between gap-3 z-30 relative shadow-md">
              
              {/* Server switch tabs */}
              <div className="flex flex-wrap items-center gap-1.5 w-full sm:w-auto">
                <span className="text-[10px] text-gray-500 uppercase tracking-widest font-black mr-1 hidden sm:inline">Opções:</span>
              ...
              <div className="flex items-center justify-between w-full sm:w-auto gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-bold text-gray-300 hover:text-white border border-gray-800 hover:border-gray-700 px-3.5 py-1.5 rounded-lg bg-black cursor-pointer transition-colors ml-auto sm:ml-0"
                >
                  <ArrowLeft className="w-3.5 h-3.5" />
                  <span>Voltar às Capas</span>
                </button>
              </div>
            </div>
          )}
