...
import { 
  ...
  Bell, Send, Radio, Wrench, ShieldAlert, Ban, Unlock, Lock, MessageSquare,
...
export default function AdminPanel({ movies, onMoviesUpdated, currentUser }: AdminPanelProps) {
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (userRoleFilter === "banned") {
      list = list.filter((u: any) => u.status === "banned" || (u.lockoutUntil && new Date(u.lockoutUntil).getTime() > Date.now()));
  ...
  // Unlock User from Login Lockout / Attempts Block
  const handleUnlockUserLockout = (userId: string, userName: string) => {
    setConfirmationModal({
      isOpen: true,
      title: "Desbloquear Tentativas de Login",
      message: `Deseja remover as restrições de tentativas incorretas e zerar a contagem para "${userName}"? O usuário poderá voltar a tentar fazer login imediatamente.`,
      confirmText: "Sim, Desbloquear Agora",
      cancelText: "Cancelar",
      isDanger: false,
      onConfirm: async () => {
        setConfirmationModal(null);
        try {
          const response = await fetch(`/api/users/${encodeURIComponent(userId)}/unlock`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "x-user-email": currentUser?.email || ""
          ...
          const data = await response.json();
          if (response.ok && data.success) {
            showToast(data.message || "Conta desbloqueada com sucesso!", "success");
            loadUsers();
          } else {
            showToast(`Erro ao desbloquear: ${data.error || "Operação não autorizada"}`, "error");
          }
        } catch (e) {
          showToast("Erro de conexão ao desbloquear usuário.", "error");
        }
      }
    });
  };
            ...
            {usersLoading ? (
              ...
              <div className="space-y-4">
                          ...
                          <tr key={u.id} className="hover:bg-gray-900/10 transition-colors">
                            ...
                            <td className="py-3 px-4">
                              <div className="flex flex-wrap items-center gap-1.5">
                                ...
                                {u.status === "banned" && (
                                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider bg-red-600/20 border border-red-500/50 text-red-400" title="Bloqueado manualmente por admin">
                                    <Ban className="w-2.5 h-2.5" />
                                    Bloqueado
                                  </span>
                                )}
                                {u.lockoutUntil && new Date(u.lockoutUntil).getTime() > Date.now() && (
                                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider bg-amber-500/20 border border-amber-500/50 text-amber-400" title={u.lockoutReason || "Bloqueado por erro de senha"}>
                                    <Lock className="w-2.5 h-2.5" />
                                    Bloqueado Tentativas ({u.failedAttempts || 3}x)
                                  </span>
                                )}
                              </div>
                            </td>
                            <td className="py-3 px-4 text-right">
                              <div className="flex items-center justify-end flex-wrap gap-1.5">
                                <button
                                  ...
                                  className="px-2.5 py-1.5 rounded-lg bg-gray-950 border border-gray-900 hover:border-gray-700 text-gray-300 hover:text-white text-[10px] font-bold transition-all cursor-pointer flex items-center gap-1 shrink-0 whitespace-nowrap"
                                ...
                                <button
                                  ...
                                  className={`text-[10px] font-bold px-2.5 py-1.5 rounded-lg border transition-all cursor-pointer shrink-0 whitespace-nowrap ${
                                    u.role === "admin"
                                ...
                                {u.lockoutUntil && new Date(u.lockoutUntil).getTime() > Date.now() && (
                                  <button
                                    onClick={() => handleUnlockUserLockout(u.id, u.name)}
                                    className="px-2.5 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30 hover:bg-amber-500/20 text-amber-300 hover:text-white text-[10px] font-bold transition-all cursor-pointer flex items-center gap-1 shrink-0 whitespace-nowrap"
                                    title="Zerar Tentativas e Liberar Conta"
                                  >
                                    <Unlock className="w-3 h-3 text-amber-400" />
                                    <span>Liberar Trava</span>
                                  </button>
                                )}
                                <button
                                  onClick={() => handleToggleUserStatus(u.id, u.status, u.name)}
                                  className={`text-[10px] font-bold px-2.5 py-1.5 rounded-lg border transition-all cursor-pointer flex items-center gap-1 shrink-0 whitespace-nowrap ${
                                    u.status === "banned"
                                ...
                                <button
                                  ...
                                  className="px-2.5 py-1.5 rounded-lg bg-red-950/20 border border-red-900/40 hover:border-red-600 hover:bg-red-900/40 text-red-400 hover:text-white text-[10px] font-bold transition-all cursor-pointer flex items-center gap-1 shrink-0 whitespace-nowrap"
                                  title="Excluir Usuário"
                                >
                                  <Trash2 className="w-3.5 h-3.5" />
                                  <span>Excluir</span>
                ...
                <div className="grid grid-cols-1 gap-3 md:hidden">
                  {paginatedUsers.map((u: any) => {
                    const hasLockout = u.lockoutUntil && new Date(u.lockoutUntil).getTime() > Date.now();
                    return (
                      <div 
                        key={u.id}
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="space-y-0.5">
                            ...
                            {u.registeredIp && (
                              <p className="text-[10px] text-gray-500 font-mono">IP: {u.registeredIp}</p>
                            )}
                          </div>
                          <div className="flex flex-col items-end gap-1">
                            ...
                            {hasLockout && (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider bg-amber-500/20 border border-amber-500/50 text-amber-400 shrink-0">
                                <Lock className="w-2.5 h-2.5" />
                                Bloqueado Tentativas ({u.failedAttempts || 3}x)
                              </span>
                            )}
                        ...
                        <div className="pt-3 border-t border-gray-900/60 grid grid-cols-2 gap-2">
                          <button
                            onClick={() => handleOpenEditUser(u)}
                            className="w-full h-9 bg-gray-950 border border-gray-900 hover:border-gray-700 text-gray-300 text-[11px] font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5"
                          >
                            <Edit className="w-3.5 h-3.5" />
                            <span>Editar</span>
                          </button>
                          ...
                          <button
                            onClick={() => handleToggleUserRole(u.id, u.role, u.name)}
                            className={`w-full h-9 text-[11px] font-bold rounded-xl border transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                              u.role === "admin"
                                ? "bg-gray-950 border-gray-900 text-gray-400 hover:text-white"
                                : "bg-red-950/20 border-red-900/30 text-red-400 hover:bg-red-900 hover:text-white"
                            }`}
                          ...
                          {hasLockout && (
                            <button
                              onClick={() => handleUnlockUserLockout(u.id, u.name)}
                              className="col-span-2 w-full h-9 bg-amber-500/10 border border-amber-500/30 text-amber-300 hover:bg-amber-500/20 text-[11px] font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5"
                              title="Liberar Trava"
                            >
                              <Unlock className="w-3.5 h-3.5 text-amber-400" />
                              <span>Liberar Trava (Desbloquear)</span>
                            </button>
                          )}

                          <button
                            onClick={() => handleToggleUserStatus(u.id, u.status, u.name)}
                            className={`w-full h-9 border text-[11px] font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                              u.status === "banned"
                                ? "bg-emerald-950/40 border-emerald-800/40 text-emerald-400 hover:bg-emerald-900 hover:text-white"
                                : "bg-red-950/40 border-red-900/40 text-red-400 hover:bg-red-900 hover:text-white"
                            }`}
                          ...
                          <button
                            onClick={() => handleDeleteUser(u.id, u.name)}
                            className="w-full h-9 bg-red-950/20 border border-red-900/40 hover:bg-red-900/40 text-red-400 hover:text-white text-[11px] font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                            <span>Excluir</span>
                          </button>
